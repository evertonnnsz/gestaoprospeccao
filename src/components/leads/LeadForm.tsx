...
const COMMERCIAL_AGENDA_TYPES = {
  commercial_meeting: 'Reunião comercial / Diagnóstico',
...
export function LeadForm({ open, onOpenChange, lead, onSuccess }: LeadFormProps) {
  ...
  const [commercialAgenda, setCommercialAgenda] = useState({
    id: '',
  ...
  const showCommercialAgenda = ['agendou_reuniao', 'proposta_enviada', 'em_negociacao'].includes(formData.status);
      ...
      if (lead) {
        ...
        if (lead.id) {
          db
            ...
            .eq('source_type', 'lead')
            .eq('lead_id', lead.id)
            .in('event_type', ['commercial_meeting', 'proposal_meeting'])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (savedLeadId && showCommercialAgenda && commercialAgenda.scheduled_date && commercialAgenda.scheduled_time) {
        const agendaPayload = {
          user_id: user.id,
          source_type: 'lead',
          lead_id: savedLeadId,
          client_id: null,
          event_type: commercialAgenda.event_type,
        ...
        const agendaRequest = commercialAgenda.id
          ? db.from('agenda_events').update(agendaPayload).eq('id', commercialAgenda.id)
          : db.from('agenda_events').insert(agendaPayload);

        const { error: agendaError } = await agendaRequest;
            ...
            <div className="space-y-4 rounded-lg border border-warning/30 bg-warning/5 p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="space-y-2">
                  <Label>Tipo</Label>
                  <Select
                    value={commercialAgenda.event_type}
                    onValueChange={(value) =>
                      setCommercialAgenda((prev) => ({
                        ...prev,
                        event_type: value as keyof typeof COMMERCIAL_AGENDA_TYPES,
                      }))
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      <SelectItem value="commercial_meeting">Diagnóstico</SelectItem>
                      <SelectItem value="proposal_meeting">Proposta</SelectItem>
                    </SelectContent>
                  </Select>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="commercial_scheduled_date">Data da reunião</Label>
                  <Input
                    id="commercial_scheduled_date"
                    type="date"
                    value={commercialAgenda.scheduled_date}
                    onChange={(e) =>
                      setCommercialAgenda((prev) => ({ ...prev, scheduled_date: e.target.value }))
                    }
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="commercial_scheduled_time">Horário</Label>
                  <Input
                    id="commercial_scheduled_time"
                    type="time"
              ...
              <div className="space-y-2">
                <Label htmlFor="commercial_agenda_notes">Observações da reunião</Label>
                <Textarea
                  id="commercial_agenda_notes"
                  rows={3}
                  placeholder="Ex: Diagnóstico, reunião de proposta, próximos pontos..."
                  value={commercialAgenda.notes}
                  onChange={(e) =>
                    setCommercialAgenda((prev) => ({ ...prev, notes: e.target.value }))
                  }
                />
              </div>
            </div>
